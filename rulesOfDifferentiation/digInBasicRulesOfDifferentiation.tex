...
\begin{expandable}
Recall that for a linear function $f(x) = mx+b$ where $m$ is the slope and $b$ is 
the $y$-intercept, $f'(x)=m$. Further for a constant $c$, the function $f(x)=c = 0x + c$,
so the slope of the straight line is $0$. This only works because $y=c$ is a straight line.
\end{expandable}

...
Suppose you are given a polynomial $f(x)$ and you are asked to compute $f'(2)$. What is the most efficient way to answer the question?
  ...
  \choice[correct]{Compute $\displaystyle \lim_{h\to 0} \frac{f{2+h} - f(2)}{h}$}
  \choice{Compute the derivative as a function $f'(x) = \lim_{h\to 0} \frac{f{x+h} - f(x)}{h}$, and then evaluate $f'(x)$ at $x=2$}
  \end{multipleChoice}
  \begin{question}
  Suppose you are given a polynomial $f(x)$ and you are asked to compute $f'(-2)$, $f'(0)$, and $f'(2)$. What is the most efficient way to answer the question?
    \begin{multipleChoice}
      \choice{Compute $\displaystyle \lim_{h\to 0} \frac{f{-2+h} - f(-2)}{h}$, 
      $\displaystyle \lim_{h\to 0} \frac{f{0+h} - f(0)}{h}$, and 
      $\displaystyle \lim_{h\to 0} \frac{f{2+h} - f(2)}{h}$}
      \choice[correct]{Compute the derivative as a function $f'(x) = \lim_{h\to 0} \frac{f{x+h} - f(x)}{h}$, and then evaluate $f'(x)$ at $x=-2,0$, and $-2$}
    \end{multipleChoice}
    \begin{feedback}
